...
export default function StockInsightModal(props: {
  ...
  // 匿名公开版:关注/一键设提醒/问AI 都是仅站长功能(对应后端受登录保护),
  // 匿名访客隐藏这些按钮,只保留图片/分享/复制/刷新等无需登录的操作。
  ...
  const loadHoldingAgg = useCallback(async () => {
    if (!symbol) return
    // 持仓信息是站长个人数据(/portfolio/summary 受登录保护)。匿名访客没有持仓,
    // 直接置空跳过,避免打开个股/深度分析时对受保护接口发起注定 401 的请求。
    if (!isAuthenticated()) {
      setHoldingAgg(null)
  ...
  const loadReports = useCallback(async () => {
    if (!symbol) return
    // 盘前/盘后简报走受登录保护的 /api/history,且「报告」tab 在匿名公开版已隐藏。
    // 匿名访客直接置空跳过,避免每次开个股弹窗对受保护接口发起注定 401 的请求。
    if (!isAuthenticated()) { setReports([]); return }
    try {
  ...
  const stopDeepPoll = useCallback(() => {
    if (deepPollRef.current) { clearInterval(deepPollRef.current); deepPollRef.current = undefined }
  }, [])

  // 轮询真实进度(驱动黑客帝国流式界面):每 2.5s 拉一次 /agents/runs/:trace/progress
  const startDeepPoll = useCallback((traceId: string) => {
    stopDeepPoll()
    ...
    deepPollRef.current = setInterval(async () => {
      tries += 1
      try {
        const p = await tradingAgentsApi.getProgress(traceId)
        setDeepProgress(p)
        if (p.status === 'success') { stopDeepPoll(); setDeepTriggering(false); loadDeepResult() }
        else if (p.status === 'failed') { stopDeepPoll(); setDeepTriggering(false) }
        else if (p.status === 'not_found' && tries > 4) { stopDeepPoll(); setDeepTriggering(false); loadDeepResult() }
        else if (p.status === 'stale' && tries > 4) { stopDeepPoll(); setDeepTriggering(false) }
      } catch { /* transient, keep polling */ }
      if (tries >= 160) { stopDeepPoll(); setDeepTriggering(false) } // ~6-7 分钟兜底
    }, 2500)
  }, [stopDeepPoll, loadDeepResult])

  // 从弹窗触发深度分析(任意股票:get-or-create 拿 stockId → 触发 → 轮询真实进度)
  const handleTriggerDeep = useCallback(async () => {
    if (!symbol || deepTriggering) return
    setDeepTriggering(true); setDeepProgress(null)
    try {
      const stocks = await stocksApi.list()
      let stock = (stocks || []).find((s) => s.symbol === symbol && s.market === market) || null
      if (!stock) stock = await stocksApi.create({ symbol, name: resolvedName || symbol, market })
      const res = await tradingAgentsApi.trigger(stock.id)
      if (res.cached) {
        // 当天已有报告,后端直接复用未重跑 → 直接加载展示
        toast('当天已有分析报告,直接展示', 'success')
        setDeepTriggering(false)
        loadDeepResult()
      } else if (res.trace_id) {
        if (res.deduplicated) toast('已有用户发起分析,为你接入实时进度', 'success')
        startDeepPoll(res.trace_id)
      } else {
        toast('已提交,稍后自动显示', 'success')
      }
    } catch (e) {
      // 后端限流/预算闸会返回 429 + 具体原因(如"每小时上限""本月额度已用尽"),原样展示给用户
      toast(e instanceof Error && e.message ? e.message : '触发失败,请稍后重试', 'error')
      setDeepTriggering(false)
    }
  }, [symbol, market, resolvedName, deepTriggering, toast, startDeepPoll])

  // 深度 tab 打开时:若该股已有在跑的分析,自动接上进度(如别处触发/刷新页面后)
  useEffect(() => {
    if (tab !== 'deep' || !props.open || deepResult || deepTriggering || deepPollRef.current) return
    tradingAgentsApi.findRunning(symbol).then((r) => {
      if (r?.trace_id && r.status === 'running') startDeepPoll(r.trace_id)
    }).catch(() => {})
  }, [tab, props.open, symbol, deepResult, deepTriggering, startDeepPoll])

  // 关闭弹窗/卸载时停止轮询
  useEffect(() => { if (!props.open) stopDeepPoll() }, [props.open, stopDeepPoll])
  useEffect(() => () => stopDeepPoll(), [stopDeepPoll])

  // 拉取默认 AI 模型名(展示在建议来源后,让用户知道判断依据的模型)。
  // 匿名公开版下 /providers 受登录保护(含密钥),故改从公开的预算接口读模型名——
  // budget.estimate_next_run.model 即下次深度分析使用的模型,不含任何密钥。
  useEffect(() => {
    if (!props.open || aiModel) return
    tradingAgentsApi.getBudget()
      .then((b) => {
        const m = b?.estimate_next_run?.model
        if (m) setAiModel(m)
      })
      .catch(() => {})
  }, [props.open, aiModel])
  ...
  useEffect(() => {
    ...
    setDeepLoaded(false)
    setDeepHistory(null)
    // 切股票时必须停掉上一只股票的深度进度轮询并清空进度状态,
    // 否则弹窗复用同一实例时会把上一只(如五粮液)的实时分析台带到下一只(如茅台)上
    stopDeepPoll()
    setDeepProgress(null)
    setDeepTriggering(false)
    loadCore()
  }, [props.open, symbol, market, loadCore, stopDeepPoll])
  ...
  const shareText = useMemo(() => {
    ...
    const lines = [
      `【Huipingce 洞察】${resolvedName}（${symbol} · ${marketLabel}）`,
      `时间：${ts}`,
      `现价：${price}（${chg}）`,
      `建议：${action}`,
      `信号：${signal}`,
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  ...
  <text x="76" y="104" fill="#93c5fd" font-size="26" font-family="-apple-system,BlinkMacSystemFont,Segoe UI,Microsoft YaHei,sans-serif">Huipingce 洞察</text>
            ...
            {tab === 'overview' && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 items-stretch">
                  ...
                  {/* 匿名公开版:隐藏「AI报告」卡(盘前/盘后定时报告,需配置监控) */}
                  {false && (
                  <div className="card p-4 h-full flex flex-col">
            ...
            {tab === 'deep' && (
              <div className="space-y-3">
                ...
                <DeepAnalysisSection
                  ...
                  onTrigger={handleTriggerDeep}
                  triggering={deepTriggering}
                  ...
                  <div className="max-h-[56vh] overflow-y-auto pr-1 scrollbar space-y-3">
                    {suggestions.map((item, idx) => (
                      <div key={`${item.created_at || 's'}-${idx}`} className="card p-4">
                        <SuggestionBadge suggestion={item} stockName={resolvedName} stockSymbol={symbol} kline={klineSummary} hasPosition={!!props.hasPosition} modelName={aiModel} />
                      </div>
                    ))}
                  </div>
                )}
...
// 阶段名 → 图标 + 中文(TradingAgents 流程)
const DEEP_STAGE_MAP: { key: string; icon: string; label: string }[] = [
  { key: 'market', icon: '📊', label: '技术面分析师' },
  { key: 'chart', icon: '📊', label: '技术面分析师' },
  { key: 'technical', icon: '📊', label: '技术面分析师' },
  { key: 'social', icon: '💬', label: '情绪分析师' },
  { key: 'sentiment', icon: '💬', label: '情绪分析师' },
  { key: 'news', icon: '📰', label: '新闻分析师' },
  { key: 'fundamental', icon: '📈', label: '基本面分析师' },
  { key: 'bull', icon: '🐂', label: '多方研究员' },
  { key: 'bear', icon: '🐻', label: '空方研究员' },
  { key: 'debate', icon: '⚔️', label: '多空辩论' },
  { key: 'research', icon: '⚔️', label: '研究员辩论' },
  { key: 'trader', icon: '💼', label: '交易员制定计划' },
  { key: 'risk', icon: '⚠️', label: '风控团队审查' },
  { key: 'portfolio', icon: '🎯', label: '基金经理(PM)决策' },
  { key: 'manager', icon: '🎯', label: '基金经理(PM)决策' },
  { key: 'final', icon: '🎯', label: '最终决策' },
  { key: 'decision', icon: '🎯', label: '最终决策' },
]
function deepStageInfo(name?: string): { icon: string; label: string } {
  const k = (name || '').toLowerCase()
  for (const s of DEEP_STAGE_MAP) if (k.includes(s.key)) return { icon: s.icon, label: s.label }
  return { icon: '◆', label: name || '分析中' }
}

// 黑客帝国风格·深度分析实时进度控制台(数字雨 + 阶段流水 + 进度)
function DeepProgressConsole({ progress }: { progress: ProgressResponse | null }) {
  ...
  useEffect(() => {
    ...
    resize()
    const chars = 'アカサタナ0123456789$¥涨跌买卖多空牛熊'.split('')
    const fs = 13
    let drops = Array(Math.ceil(c.width / fs)).fill(1)
    const timer = setInterval(() => {
      ctx.fillStyle = 'rgba(0,12,5,.12)'
  ...
  const pct = stages.length ? Math.max(6, Math.round((done / stages.length) * 100)) : (progress ? 8 : 4)
  const toolkit = (progress?.toolkit_recent || []).slice(-6)
  const elapsed = progress?.elapsed_sec ? Math.round(progress.elapsed_sec) : 0
  const cost = progress?.total_cost_usd || 0

  return (
    <div className="relative overflow-hidden rounded-xl border border-[#1a7a4d] bg-[#02100a]">
      <canvas ref={canvasRef} className="pointer-events-none absolute inset-0 h-full w-full opacity-30" />
      <div className="relative z-10 p-4 font-mono text-[#7fffb5]">
        <div className="mb-3 flex items-center gap-1.5 text-[12px]">
          <span className="h-2.5 w-2.5 rounded-full bg-red-500" />
          <span className="h-2.5 w-2.5 rounded-full bg-yellow-400" />
          <span className="h-2.5 w-2.5 rounded-full bg-green-400" />
          <span className="ml-1 tracking-widest text-[#9affc8]">QUANT·ENGINE — AI 投研团队实时分析中</span>
          <span className="ml-auto animate-pulse text-red-400">● REC</span>
        </div>
        <div className="space-y-1.5 text-[13px]">
          {stages.length === 0 ? (
            <div className="text-[#4fbf8a]">▸ 正在唤醒投研团队 · 连接海内外数据源…</div>
          ) : (
            stages.map((s, i) => {
              const info = deepStageInfo(s.name)
              const icon = s.status === 'done' ? '✓' : s.status === 'running' ? '⟳' : '·'
              const cls = s.status === 'done' ? 'text-[#5effa6]' : s.status === 'running' ? 'text-[#eaffe9] font-semibold' : 'text-[#3a7a55]'
              return (
                <div key={i} className={`flex items-center gap-2 ${cls}`}>
                  <span className={s.status === 'running' ? 'inline-block animate-spin' : ''}>{icon}</span>
                  <span>{info.icon} {info.label}</span>
                  {s.status === 'running' && <span className="animate-pulse text-[11px]">…推演中</span>}
                  {s.status === 'done' && s.duration_sec ? <span className="text-[11px] text-[#3a7a55]">· {Math.round(s.duration_sec)}s</span> : null}
                </div>
              )
            })
          )}
        ...
        {toolkit.length > 0 && (
          <div className="mt-2.5 space-y-0.5 border-t border-[#1a7a4d]/40 pt-2 text-[11px] text-[#4fbf8a]">
            {toolkit.map((t, i) => (
              <div key={i} className="truncate">◆ 抓取 {t.method || t.action} {t.symbol || ''}{t.source ? ` · ${t.source}` : ''}</div>
            ))}
          </div>
        )}
        <div className="mt-3">
          <div className="h-1.5 overflow-hidden rounded bg-[#22ff88]/15">
            <div className="h-full rounded bg-gradient-to-r from-[#0f9] to-[#5effa6] transition-all duration-500" style={{ width: `${pct}%`, boxShadow: '0 0 10px rgba(34,255,136,.8)' }} />
          </div>
          <div className="mt-1.5 flex justify-between text-[11px] text-[#5fb98a]">
            <span>{progress?.current_stage ? `当前 · ${deepStageInfo(progress.current_stage).label}` : '4 分析师 → 多空辩论 → 风控 → PM 决策'}</span>
            <span>{elapsed}s · ${cost.toFixed(3)} · {pct}%</span>
          </div>
        </div>
      </div>
    </div>
...
function DeepAnalysisSection({
  ...
  if (triggering) {
    return <DeepProgressConsole progress={progress || null} />
  }
  if (!result && !history?.items?.length) {
    return (
      <div className="card p-6 text-center text-[12px] text-muted-foreground space-y-3">
        <div>暂无深度分析报告</div>
        <div className="text-[11px] text-muted-foreground/70">
          点击下方按钮,启动 AI 投研团队(4 类分析师 + 多空辩论 + PM 决策)
        </div>
        {onTrigger && (
          <Button onClick={onTrigger} className="mt-1">
            🧠 开始深度分析（约 3-5 分钟）
          </Button>
        )}
      </div>
