...
import { RefreshCw, AlertTriangle, Sparkles, Activity, ShieldAlert, Newspaper, Share2, Search } from 'lucide-react'
import {
  fetchAPI,
  isAuthenticated,
  dashboardApi,
...
// 指数价格实时跳动:数值变化时红涨绿跌闪一下(A股习惯)
function TickPrice({ value }: { value: number }) {
  ...
  return (
    <span className={`font-mono text-foreground/80 tick-price ${flash ? `tick-${flash}` : ''}`}>
      {value.toFixed(2)}
    </span>
  )
}
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    // 公开快车道:指数/盘中扫描/机会概览——匿名可访问,先让首屏尽快出来
    const [idx, sc, ov] = await Promise.allSettled([
    ...
    // 个人数据(持仓体检/提醒/待办/基准/归因)仅登录站长可见。
    // 匿名公开版这些区块已隐藏,故直接跳过拉取,避免每次首屏刷一堆 401 噪音。
    if (isAuthenticated()) {
      Promise.allSettled([
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      dashboardApi.indices().then((v) => setIndices(v)).catch(() => {})
    }, 4000)
  ...
  const doSearch = async (q: string) => {
    ...
    setSearching(true)
    try {
      const results = await fetchAPI<{ symbol: string; name: string; market: string }[]>(
  ...
  const pickSearch = (r: { symbol: string; name: string; market: string }) => {
    setSearchOpen(false); setSearchQuery('')
    openStock(r.symbol, r.market || 'CN', r.name)
  }
    ...
    <div className="page-container pb-10">
      ...
      <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px]">
          {false && hasHoldings && portfolioPnlPct != null && (
          ...
          {indices.slice(0, 5).map((ix) => (
            <span key={`${ix.market}:${ix.symbol}`} className="flex items-center gap-1">
              ...
              {ix.current_price != null && (
                <TickPrice value={ix.current_price} />
              )}
      ...
      {/* 顶部股票搜索 · 公开版核心入口:搜股票 → 开详情 → AI 深度分析 */}
      <div className="relative mb-4">
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-card px-4 py-3 shadow-sm transition focus-within:border-primary focus-within:shadow-md">
          <Search className="h-5 w-5 shrink-0 text-muted-foreground" />
          <input
            value={searchQuery}
            onChange={(e) => onSearchInput(e.target.value)}
            onFocus={() => searchResults.length > 0 && setSearchOpen(true)}
            placeholder="输入股票代码或名称，AI 帮你深度分析（如 600519 / 贵州茅台 / 宁德时代）"
        ...
        {searchOpen && searchResults.length > 0 && (
          <div className="absolute z-30 mt-1 w-full overflow-hidden rounded-xl border border-border bg-card shadow-lg">
            {searchResults.slice(0, 8).map((r) => (
              ...
              >
                <span className="rounded bg-primary/10 px-1.5 py-0.5 text-[10px] text-primary">{r.market}</span>
                <span className="text-[14px] font-medium">{r.name}</span>
                <span className="font-mono text-[12px] text-muted-foreground">{r.symbol}</span>
              </div>
            ))}
          </div>
        )}
            ...
            <div className="grid grid-cols-1 gap-x-8 sm:grid-cols-2">
              {opportunities.slice(0, 6).map((o) => (
                <div
                  ...
                  className="flex cursor-pointer items-center gap-2 border-b border-border/40 py-2 hover:bg-accent/30"
