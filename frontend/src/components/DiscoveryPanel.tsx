...
export default function DiscoveryPanel({ monitorStocks, onOpenStock }: Props) {
  ...
  // 匿名公开版:默认涨幅榜(去掉需登录的 For You 个性化)
  const [stocksMode, setStocksMode] = useLocalStorage<'turnover' | 'gainers' | 'for_you'>('panwatch_dashboard_stocksMode', 'gainers')
  // 老用户可能存了 for_you,重置为涨幅榜
  useEffect(() => { if (stocksMode === 'for_you') setStocksMode('gainers') }, [])
  ...
  useEffect(() => {
    ...
    // 持仓高亮是站长个人数据(/portfolio/summary 受登录保护),匿名访客跳过,避免 401 噪音
    if (isAuthenticated()) {
